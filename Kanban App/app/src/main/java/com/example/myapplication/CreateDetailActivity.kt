...
class CreateDetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
       ...
       if(edit) {
        ...
        val c = Calendar.getInstance() //calendar
        val year = c.get(Calendar.YEAR)
        val month = c.get(Calendar.MONTH)+1
        val day = c.get(Calendar.DAY_OF_MONTH)
        val textview2 = findViewById<TextView>(R.id.due_date)
        val textview3 = findViewById<TextView>(R.id.content)
        ...
        if(edit){
            ...
            db.collection("users").document(userId!!).get().addOnCompleteListener { it2 ->
                if (it2.isSuccessful) {
                    ...
                    val data = userData.get(section) as java.util.ArrayList<HashMap<String, Any>>
